#!/usr/bin/env python3
"""
Project ValiMeli — Theedhum Nandrum Downstream Evaluation with Best Respective Models
Evaluates:
  - Tamil: Multilingual-A1-MT (ValiMeli Multi-Task 3.2M Model)
  - Malayalam: Multilingual-A0 (IndicXlit Baseline 3.2M Model)
on the DravidianCodeMix FIRE 2020 YouTube Comments Sentiment Dataset.
"""
...
class TransliteratorWrapper:
    def __init__(self, checkpoint_path: str, device: torch.device):
        ...
        self.model = InferenceTransformer(
            self.src_vocab.n, self.tgt_vocab.n,
            d_model=256, nhead=4, num_layers=6, dim_feedforward=1024
        ).to(device)
        
        state_dict = ckpt["model_state_dict"]
        # Filter out auxiliary classification heads if present
        filtered_dict = {k: v for k, v in state_dict.items() if not k.startswith("aux_phono_proj")}
...
def run_sentiment_eval(lang: str, model_path: str, model_name: str):
    ...
    print(f"\nEvaluating {full_lang.upper()} Sentiment using Best Model: {model_name}...", flush=True)
    
    engine = TransliteratorWrapper(model_path, device)
    ...
    # Dual-stream concatenation: Raw Text + Best Neural Transliteration
    train_augmented = [f"{r} [BEST_STREAM] {t}" for r, t in zip(train_raw_texts, train_trans_texts)]
    dev_augmented = [f"{r} [BEST_STREAM] {t}" for r, t in zip(dev_raw_texts, dev_trans_texts)]
    ...
    print(f" ► [{full_lang.upper()}] Best Model ({model_name}) Results:")
    print(f"    Macro F1:    {macro_f1:.2f}%")
    print(f"    Weighted F1: {weighted_f1:.2f}%")
    print(f"    Accuracy:    {acc:.2f}%")
    print(f"    Classes:     Positive={pos_f1:.2f}% | Negative={neg_f1:.2f}% | Mixed={mix_f1:.2f}%\n", flush=True)
...
def main():
    print("=" * 80)
    print(" EVALUATING BEST RESPECTIVE MODELS ON THEEDHUM NANDRUM SENTIMENT BENCHMARK")
    print("=" * 80)
    
    # 1. Tamil with Multilingual-A1-MT (Our best Tamil model)
    tam_ckpt = os.path.join(RUNS_DIR, "multilingual_multitask_scaling_best.pt")
    tam_res = run_sentiment_eval("tam", tam_ckpt, "ValiMeli Multilingual-A1-MT (3.2M)")
    
    # 2. Malayalam with Multilingual-A0 (Our best Malayalam model)
    mal_ckpt = os.path.join(RUNS_DIR, "multilingual_scaling_best.pt")
    mal_res = run_sentiment_eval("mal", mal_ckpt, "IndicXlit Multilingual-A0 (3.2M)")
    
    all_res = {"tamil": tam_res, "malayalam": mal_res}
    with open(os.path.join(ARTIFACTS_DIR, "theedhum_nandrum_best_models_results.json"), "w") as f:
        json.dump(all_res, f, indent=2)
    print("⭐ Saved results to artifacts/theedhum_nandrum_best_models_results.json")
